...
#. Retrieves certificates (``--username=$BADC_USERNAME``). Password read from the pipe (``--password_from_pipe``).
...
If the data looks OK, then one can use the validate file to bypass the ``ask`` and ``validate`` steps::
    ...
    echo $BADC_PASSWORD | cdb_query CMIP5 reduce_soft_links download_opendap reduce \
                      --username=$BADC_USERNAME \
                      --password_from_pipe \
                      --out_destination=./out/CMIP5/ \
                      --num_procs=10 \
