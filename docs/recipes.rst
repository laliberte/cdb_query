...
* ``cdb_query_CMIP5 ask``, searches the CMIP5 archive and produces a file with pointers to the data. 
* ``cdb_query_CMIP5 validate`` can be used to find all the experiments that have all of
  the avialalble years. This command outputs a file that points to data for every month of the requested period.
* ``cdb_query_CMIP5 download``, reads the output from ``cdb_query_CMIP5 validate``,
