...
class browser:
    def __init__(self, search_path, options):
        ...
        self.search_path = os.path.abspath(
                                os.path.expanduser(
    ...
    def descend_tree(self, database, list_level=None):
        ...
        if self.file_type in database.header['file_type_list']:
            ...
            if hasattr(database.drs, 'alt_base_drs'):
                (only_list
                 .append(descend_tree_recursive(database, file_expt_copy,
                                                [item for item
...
def descend_tree_recursive(database, file_expt, tree_desc, top_path,
                           options, list_level=None, alt=False):
    ...
    for subdir in get_immediate_subdirectories(top_path):
        # Include only subdirectories that were specified if
        # this level was specified:
        if (db_utils
