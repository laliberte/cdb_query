...
def descend_tree_recursive(database, file_expt, tree_desc, top_path, options,
                           session, list_level=None, alt=False):
    ...
    for subdir in http_list_dirs(session, top_path):
        # Include only subdirectories that were specified if this level
        # was specified:
        if (db_utils
...
def http_list_dirs(session, url, ext=''):
    ...
    return [node.get('href')[:-1] for node in soup.find_all('a')
            if (node.get('href').endswith('/') and
