...
def recover_time(file,semaphores):
    file_name=str(file['path']).replace('fileServer','dodsC').split('|')[0]
    dataset=remote_netCDF(file,semaphores)
...
class netCDF_pointers:
    ...
    def record_header(self):
        for value in self.header.keys():
            output_root.setncattr(value,json.dumps(self.header[value]))
        #Put version:
        output_root.setncattr('cdb_query_file_spec_version','1.0')
    ...
    def record_meta_data(tree,paths_list,var,time_frequency,experiment):
        if not time_frequency in ['fx','clim']:
            ...
            self.retrieve_time_and_meta_data(tree,paths_list,var,experiment)
        else:
    ...
    def record_paths(tree,paths_list,var,time_frequency,experiment):
        ...
        paths_ordering = self.order_paths_by_preference(paths_list,sorts_list,id_list)
        self.create_netcdf_pointers_file(tree,paths_list,paths_ordering,id_list)
        return

    def create_netcdf_pointers_file(tree,source_files,paths_ordering,id_list):
...
class paths_ordering:
    ...
    def order_paths_by_preference(self,paths_list):
        #FIND ORDERING:
        paths_desc=[]
